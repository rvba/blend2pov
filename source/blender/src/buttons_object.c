/**
 * $Id: buttons_object.c 14340 2008-04-06 13:44:50Z stiv $
 *
...
static void object_panel_object(Object *ob)
{
	...
	if (G.scene->r.renderer>=R_YAFRAY) {
		uiDefButBitI(block, TOG, 1, B_REDR, "no image",		10,10,95,20, &ob->povViz, 0, 0, 0, 0, "Make object invisible");
		uiDefButBitI(block, TOG, 1, B_REDR, "no shadow",		105,10,95,20, &ob->povShad, 0, 0, 0, 0, "Make object invisible");
		uiDefButBitI(block, TOG, 1, B_REDR, "no reflection",		200,10,96,20, &ob->povMir, 0, 0, 0, 0, "Make object invisible");
	}
