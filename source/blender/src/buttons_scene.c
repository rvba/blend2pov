/**
 * $Id: buttons_scene.c 14235 2008-03-25 09:40:06Z campbellbarton $ 
 *
...
static void render_panel_render(void)
{
	...
	uiDefButS(block, MENU, B_SWITCHRENDER, "Rendering Engine %t|Blender Internal %x0|YafRay %x1|Povray %x2", 
												369, 142, 191, 20, &G.scene->r.renderer, 0, 0, 0, 0, "Choose rendering engine");	
#else
	uiDefButS(block, MENU, B_SWITCHRENDER, "Rendering Engine %t|Blender Internal %x0|Povray %x1", 
												369, 142, 191, 20, &G.scene->r.renderer, 0, 0, 0, 0, "Choose rendering engine");	
	...
	/*milo*/
	/* border */
	if (G.scene->r.renderer==R_POVRAY)
	{
		uiDefButBitI(block, TOG, R_POVBORDER, REDRAWVIEWCAM, "Pov-Border",	565,13,122,20, &G.scene->r.mode, 0, 0, 0, 0, "Render a small cut-out of the image (Shift+B to set in the camera view)");
	}else {
...
/*milo*/
/* radiosity */
static void checkToPovTexts ()
{
    Text *txt_iter;
    txt_iter = G.main->text.first;
    int pmat=1,pcam=1,prad=1,pphot=1,pini=1,pini2=1;
    while( ( txt_iter ) ){
        if( strcmp( "PovMat", txt_iter->id.name+2) == 0 ) pmat=0;
        if( strcmp( "PovCam", txt_iter->id.name+2) == 0 ) pcam=0;
        if( strcmp( "PovGlobalRad", txt_iter->id.name+2) == 0 ) prad=0;
        if( strcmp( "PovGlobalPhot", txt_iter->id.name+2) == 0 ) pphot=0;
        if( strcmp( "PovInit", txt_iter->id.name+2) == 0 ) pini=0;
        if( strcmp( "PovCommand-line", txt_iter->id.name+2) == 0 ) pini2=0;
        txt_iter = txt_iter->id.next;
    }
    if (pmat){
        txt_iter=add_empty_text();
        rename_id( &txt_iter->id,"PovMat" );
    }
    if (pcam){
        txt_iter=add_empty_text();
        rename_id( &txt_iter->id,"PovCam" );
    }
    if (prad){
        txt_iter=add_empty_text();
        rename_id( &txt_iter->id,"PovGlobalRad" );
    }
    if (pphot){
        txt_iter=add_empty_text();
        rename_id( &txt_iter->id,"PovGlobalPhot" );
    }
    if (pini){
        txt_iter=add_empty_text();
        rename_id( &txt_iter->id,"PovInit" );
    }
    if (pini2){
        txt_iter=add_empty_text();
        rename_id( &txt_iter->id,"PovCommand-line" );
    }
}

static void render_panel_povrayGlobal()
{
	uiBlock *block;
	block= uiNewBlock(&curarea->uiblocks, "render_panel_povrayGlobal", UI_EMBOSS, UI_HELV, curarea->win);
	uiNewPanelTabbed("Render", "Render");
	if(uiNewPanel(curarea, block, "PovRay", "Render", 320, 0, 318, 204)==0) return;
	uiDefBut(block, LABEL, 0, " ", 305,180,10,10, 0, 0, 0, 0, 0, "");
    if ((G.scene->r.PovRadio!=0) || (G.scene->r.povmode & R_POV_PHOTONS))//(G.scene->r.PovPhot==0))
        uiDefButBitI(block, TOG, R_POV_SAVE_RAD_PH, B_REDR,  "Bake Radio&Phot", 5,190,150,20,&(G.scene->r.povmode), 0, 0, 0, 0,  "Create a Bake for Radiosity and Photon map if this option are selected");
    uiDefButBitI(block, TOG, R_POV_LOAD_RAD_PH, B_REDR, "Load Previous Bake", 160,190,150,20,&(G.scene->r.povmode), 0, 0, 0, 0,  "It load Previous Baked information for Radiosity and Photon map");
    uiDefButS(block, MENU, B_REDR, "Rad.Typ%t|No Radiosity%x0|Default Radiosity%x1|Fast Radiosity%x2|Normal Radiosity%x3|2Bounce Radiosity%x4|Final Radiosity%x5|OutdoorLQ Radiosity%x6|OutdoorHQ Radiosity%x7|OutdoorLight Radiosity%x8|IndoorLQ Radiosity%x9|IndoorHQ Radiosity%x10", 5,165,150,20, &G.scene->r.PovRadio, 0, 0, 0, 0, "Global Radiosity Method");
    uiDefButBitI(block, TOG, R_POV_NOT_SEND_MESH, B_REDR, "NotExpMesh", 160,165,150,20,&(G.scene->r.povmode), 0, 0, 0, 0,   "Do not Export mesh");
    uiDefButBitI(block, TOG, R_POV_PHOTONS, B_REDR, "Photons", 5,140,150,20,&(G.scene->r.povmode), 0, 0, 0, 0,   "It activate Photons for caustics (if not activate it use fake caustics)");
    uiDefButBitI(block, TOG, R_POV_JITTER, B_REDR, "Jitter", 160,140,150,20,&(G.scene->r.povmode), 0, 0, 0, 0, "Random factor for area light");
    uiDefButF(block, NUMSLI, B_DIFF, "Default Ambient :", 5,115,150,20, &G.scene->r.PovDefaultAmbient, 0.0001, 10.0, 0, 0, "Default Ambient");
    uiDefButBitI(block, TOG, R_POV_EXECUTE, B_REDR, "NoExec.PovRay", 160,115,150,20,&(G.scene->r.povmode), 0, 0, 0, 0,  "Execute PovRay. If not selected it generate the ini and pov files on the YFexport path to render by hand");
	uiDefButBitS(block, TOGN, 1, B_REDR, "Auto AA", 5,85,150,20, &G.scene->r.YF_AA,0, 0, 0, 0, "Set AA using OSA and GI quality, disable for manual control");
	if(G.scene->r.YF_AA){
		uiDefButI(block, NUM, B_DIFF, "AA Samples ", 160,85,150,20, &G.scene->r.YF_AAsamples, 0, 2048, 10, 10, "Number of samples per pass");
		uiDefButI(block, NUM, B_DIFF, "AA Passes ", 5,60,150,20, &G.scene->r.YF_AApasses, 0, 64, 10, 10, "Number of AA passes (0 is no AA)");
		uiDefButF(block, NUMSLI, B_DIFF, "Thr ", 160,60,150,20, &G.scene->r.YF_AAthreshold, 0.000001, 1.0, 0, 0, "AA threshold");
	}
	uiDefButI(block, NUM, B_DIFF, "Raydepth ", 5,35,150,20,	&G.scene->r.YF_raydepth, 1.0, 80.0, 10, 10, "Maximum render ray depth from the camera");
	/*uiDefButI(block, NUM, B_DIFF, "Processors:", 160,35,150,20, &G.scene->r.YF_numprocs, 1.0, 8.0, 10, 10, "Number of processors to use");*/
	uiDefButF(block, NUMSLI, B_DIFF, "Gam ", 5,10,150,20, &G.scene->r.YF_gamma, 0.001, 5.0, 0, 0, "Gamma correction, 1 is off");
	uiDefButF(block, NUMSLI, B_DIFF, "Exp ", 160,10,150,20,&G.scene->r.YF_exposure, 0.0, 10.0, 0, 0, "Exposure adjustment, 0 is off");
}
...
static void render_panel_povrayOptions()
{

	uiBlock *block;
	uiBut *but;
	block = uiNewBlock(&curarea->uiblocks, "render_panel_povrayOptions", UI_EMBOSS, UI_HELV, curarea -> win);
	uiNewPanelTabbed("Render","Render");
	if(uiNewPanel(curarea, block, "Options", "Render", 320, 0, 318, 204)==0) return;
   	uiDefButBitI(block, TOG, R_POV_SHIFT, B_REDR, "Shift", 	10,	10,	150,	20,	&(G.scene->r.povshift), 0, 0, 0, 0,  "shift on/off");
	uiDefButBitI(block, TOG, R_POV_BETA, B_REDR, "beta", 	10,	160,	150,	20,	&(G.scene->r.povbeta), 0, 0, 0, 0,  "");
	uiDefButI(block,NUM,B_DIFF,"ini   ",			10,	40,	150,	20,	&(G.scene->r.povINI),0,100,0,0,"aa");	
...
void render_panels()
{
	...
	if (G.scene->r.renderer==R_POVRAY) {
		...
		if (G.scene->r.GIpower==0.0) G.scene->r.GIpower=1.0;
		if (G.scene->r.GIindirpower==0.0) G.scene->r.GIindirpower=1.0;
		if (G.scene->r.PovDefaultAmbient==0.0){
				    G.scene->r.povmode=R_POV_JITTER;
					G.scene->r.PovRadio=2;
					G.scene->r.PovDefaultAmbient=0.1;
					strcpy(G.scene->r.povAislatedObject,"");
				}
		checkToPovTexts();
		render_panel_povrayGlobal();
		render_panel_povrayOptions();
	}
#endif
