/**
 * $Id: buttons_shading.c 14296 2008-03-31 19:57:40Z jhk $ 
 *
...
static void lamp_panel_yafray(Object *ob, Lamp *la)
{
	...
	if(la->type==LA_SUN)
	{
		uiDefButBitI(block, TOG, 1, B_REDR, "SUN",130,180,80,19,&la->povsun, 0, 0, 0, 0, "");

		if (la->povsun) {
		uiBlockBeginAlign(block);
		uiDefButI(block, NUM, B_DIFF, "Dome Size:", 10,150,145,19, &la->povsun_size, 0, 10, 0, 0, "");
		uiDefButI(block, NUM, B_DIFF, "Turbidity:", 10,130,145,19, &la->povsun_turbidity, 0, 10, 0, 0, "");
		uiDefButF(block, NUM, B_DIFF, "Intensity:", 10,110,145,19, &la->povsun_mult, 0.0, 10.0, 0, 0, "");
		uiDefButF(block, NUM, B_DIFF, "Tesselation:", 10,90,145,19, &la->povsun_tesselation, 0.0, 5.0, 1, 0, "");
		uiDefButI(block, NUM, B_DIFF, "Vertices Max:", 10,70,145,19, &la->povsun_max, 10, 10000, 0, 0, "");
		uiDefButI(block, NUM, B_DIFF, "Vertices Min:", 10,50,145,19, &la->povsun_min, 1, 100, 0, 0, "");
		uiDefButI(block, TOG, B_DIFF, "Under Horizon", 10,30,145,19, &la->povsun_corr, 0, 1, 0, 0, "");
		uiDefButF(block, NUM, B_DIFF, "Horzion Epsilon:", 10,10,145,19, &la->povsun_hor, 0.1, 10, 0, 0, "");
		uiBlockEndAlign(block);
	
		uiBlockBeginAlign(block);
		uiDefButI(block, NUM, B_DIFF, "Sun Weight", 156,150,145,19, &la->povsun_sun_weight, 0, 10, 0, 0, "");
		uiDefButI(block, NUM, B_DIFF, "Horizon weight", 156,130,145,19, &la->povsun_horizon_weight, 0, 10, 0, 0, "");
		uiDefButI(block, NUM, B_DIFF, "Zenith weight", 156,110,145,19, &la->povsun_zenith_weight, 0, 10, 0, 0, "");
		uiBlockEndAlign(block);

		}

//povsun_sun_weight,povsun_horizon_weight,povsun_zenith_weight;
	...
	if (la->type==LA_LOCAL) {
		...
		uiDefButI(block, NUM, B_DIFF, "Power Fading:", 200,115,100,19, &la->pov_light_power_fading, 0, 2, 1, 0, "Sets light glow type");
		/*milo*/
		/* projected_throw */
		uiDefButI(block, TOG, B_REDR, "Projected", 50,95,40,19,&la->pov_light_projected_toggle,0,1,0,0,"");
		uiDefButC(block, TEX, B_REDR, "",	100,95,200,19, la->pov.name+2, 0.0, 21.0, 0, 0, "");
	}
...
/* POVRAY MATERIAL*/
/* > headerbuttons.c*/
static void material_panel_povray(Material *ma)
{
	uiBlock *block;
	
	block=uiNewBlock(&curarea->uiblocks,"material_panel_povray",UI_EMBOSS,UI_HELV,curarea->win);
	uiNewPanelTabbed("Material","Material");
	if(uiNewPanel(curarea, block, "Povray", "Material", 640, 0, 318, 204)==0) return;
	uiDefButBitI(block, TOG, 1, B_REDR, "Image Pattern",1,180,150,19,&ma->povmat_image_pattern[0], 0, 0, 0, 0, "");
	uiDefButF(block, NUM, B_REDR, "Blur Amount ",			1,10,150,19, &ma->pov_mat_blur_amount, 0.0, 10.0, 0, 0, "");
	uiDefButI(block, NUM, B_REDR, "Blur Samples",			1,30,150,19, &ma->pov_mat_blur_samples, 0, 200, 0, 0, "");
	uiDefButBitI(block, TOG, 1, B_REDR, "Blur",	1,50, 150,19, &ma->pov_mat_blur_toggle, 0, 0, 0, 0, "");
	/*blur */

	if (ma->povmat_image_pattern[0])
	{
		...
		xco= std_libbuttons(block, 10, 160, 0, NULL, B_MATBROWSE, ID_MA, 0, id, idfrom, &(G.buts->menunr), B_MATALONE, B_MATLOCAL, B_MATDELETE, NULL, NULL);
	}
}
