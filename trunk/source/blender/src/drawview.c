/**
 * $Id: drawview.c 14201 2008-03-21 17:13:35Z campbellbarton $
 *
...
static void drawviewborder(void)
{
	...
	if(G.scene->r.renderer==R_POVRAY)
	{
		if(G.scene->r.mode & R_POVBORDER)
		{
			...
			cpack(0);
			x3= x1+ G.scene->r.povborder.xmin*(x2-x1);
			y3= y1+ G.scene->r.povborder.ymin*(y2-y1);
			x4= x1+ G.scene->r.povborder.xmax*(x2-x1);
			y4= y1+ G.scene->r.povborder.ymax*(y2-y1);
