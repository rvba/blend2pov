/**
 * $Id: editview.c 14224 2008-03-24 09:00:39Z campbellbarton $
 *
...
void set_render_border(void)
{
	...
	if(val) {
		...
		if ( G.scene->r.renderer==R_POVRAY)
		{
			...
			G.scene->r.povborder.xmin= ((float)rect.xmin-vb.xmin)/(vb.xmax-vb.xmin);
			G.scene->r.povborder.ymin= ((float)rect.ymin-vb.ymin)/(vb.ymax-vb.ymin);
			G.scene->r.povborder.xmax= ((float)rect.xmax-vb.xmin)/(vb.xmax-vb.xmin);
			G.scene->r.povborder.ymax= ((float)rect.ymax-vb.ymin)/(vb.ymax-vb.ymin);
					
			CLAMP(G.scene->r.povborder.xmin, 0.0, 1.0);
			CLAMP(G.scene->r.povborder.ymin, 0.0, 1.0);
			CLAMP(G.scene->r.povborder.xmax, 0.0, 1.0);
			CLAMP(G.scene->r.povborder.ymax, 0.0, 1.0);
		...
		if ( G.scene->r.renderer==R_POVRAY)
		{
			...
			if ((G.scene->r.povborder.xmin <= 0.0 && G.scene->r.povborder.xmax >= 1.0 &&
				G.scene->r.povborder.ymin <= 0.0 && G.scene->r.povborder.ymax >= 1.0) ||
			   (G.scene->r.povborder.xmin == G.scene->r.povborder.xmax ||
				G.scene->r.povborder.ymin == G.scene->r.povborder.ymax ))
			{
				G.scene->r.mode &= ~R_POVBORDER;
			} else {
				G.scene->r.mode |= R_POVBORDER;
			}
