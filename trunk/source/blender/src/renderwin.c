/**
 * $Id: renderwin.c 13958 2008-03-03 16:38:27Z blendix $
 *
...
static char *renderwin_get_title()
{
	...
	if(BIF_show_render_spare()) {
		...
		else if (G.scene->r.renderer==R_POVRAY) title = "PovRay:Render (previous)";
		else title = "Blender:Render (previous)";
	}
	else {
		if (G.scene->r.renderer==R_YAFRAY) title = "YafRay:Render";
		else if (G.scene->r.renderer==R_POVRAY) title = "PovRay:Render";
